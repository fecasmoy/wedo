 ...
 * &lt;property name="
 * <a href="http://www.opensymphony.com/quartz/api/org/quartz/CronTrigger.html">
 * cronExpression</a>" value="0 0 2 * * ?" /&gt;<br>
 * &lt;property name="jobDetail"&gt;
 <ul>
 &lt;bean class="frameworkx.springframework.scheduling.quartz.
 <b>MethodInvokingJobDetailFactoryBean</b>"&gt;
 <ul>
 ...
 * </code> In this example we created a
 * MethodInvokingJobDetailFactoryBean, which will produce a JobDetail Object
 * with the jobClass property set to StatefulMethodInvokingJob.class
 * (concurrent=="false"; Set to MethodInvokingJob.class when
 * concurrent=="true"), which will in turn invoke the static <code>fooBar</code>
 * () method of the " <code>example.ExampleImpl</code>" Class. The Scheduler is
 * the heart of the whole operation; without it, nothing will happen.
 * <p>
    ...
    implements FactoryBean, BeanNameAware, InitializingBean
{
    ...
    private static final Log logger =
        LogFactory.getLog(MethodInvokingJobDetailFactoryBean.class);
     ...
     * 
     *      
     *      href="http://www.opensymphony.com/quartz/wikidocs/TutorialLesson3.html"
     *      >http://www.opensymphony.com/quartz/wikidocs/TutorialLesson3.html</a
     *      >
     * @see #afterPropertiesSet()
     */
    ...
    /**
     * judge the ojbect whether singleton
     * 
     * @return true
        ...
        throws Exception
    {
        try
        {
            ...
            logger.info("Created JobDetail: " + jobDetail + "; targetClass: "
                ...
                + staticMethod + "; arguments: " + Arrays.toString(arguments)
                + ";");
        }
        finally
        {
     ...
     * 
     * @param jobListenerNames jobListenerNames
     * @see #jobListenerNames
     */
     ...
     * 
     * @param shouldRecover shouldRecover
     * @see #shouldRecover
     */
        ...
        implements Job
    {
         ...
         */
        private static final Log logger =
            LogFactory.getLog(MethodInvokingJob.class);
            ...
            throws JobExecutionException
        {
            try
            {
                logger.debug("start");
                String targetClass =
                    context.getMergedJobDataMap().getString("targetClass");
                ...
                Object targetObject =
                    context.getMergedJobDataMap().get("targetObject");
                logger.debug("targetObject is " + targetObject);
                String targetMethod =
                    context.getMergedJobDataMap().getString("targetMethod");
                logger.debug("targetMethod is " + targetMethod);
                String staticMethod =
                    context.getMergedJobDataMap().getString("staticMethod");
                logger.debug("staticMethod is " + staticMethod);
                Object[] arguments =
                    (Object[]) context.getMergedJobDataMap().get("arguments");
                logger.debug("arguments are " + Arrays.toString(arguments));
